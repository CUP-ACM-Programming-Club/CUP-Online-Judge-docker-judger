...
function socketMessage(options, status, time, memory, pass_point, compile_msg = "", test_run_result = "") {
...
*/
	...
	try {
		...
		for (let i in options.file_stdin) {
			if (path.basename(options.file_stdin[i]).indexOf("custominput") === -1) {
		...
		*/
		...
		for (let i in options.file_stdin) {
			...
			++pass_point;
			socketMessage(options, 3, 0, 0, pass_point);
		}
		let time_usage = 0, memory_usage = 0;
		for (let i in _result) {
			time_usage = Math.max(_result[i].time_usage, time_usage);
